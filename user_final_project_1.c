	...
	while(1) {
		...
		if ((tskcount%6)==0) {
			if (GET_LVDATA_TO_LINUX) {
				...
				ptrshrdmem->DSPSend_size = sprintf(toLinuxstring,"%.1f %.1f %.1f %.1f",ROBOTps.x, ROBOTps.y, ROBOTps.theta, statePos);
...
{
	if (fabsf((destY-curY)/(destX-curX)-slope) < slopeThreshold) return 1;
	else return 0;
}
...
//	robotdest[0].x = 5;		robotdest[0].y = -3; // Point 4
...
//	robotdest[0].x = -5;	robotdest[0].y = -3; // Point 1
//	robotdest[1].x = 0;		robotdest[1].y = -1; // Home base (enter course)
//	robotdest[2].x = 0;		robotdest[2].y = 1; // Home base (enter course)
//	robotdest[3].x = 3;		robotdest[3].y = 7; // Point 2
//	robotdest[4].x = -3;	robotdest[4].y = 7; // Point 3
//	robotdest[5].x = 0;		robotdest[5].y = 1; // Home base (enter course)
//	robotdest[6].x = 0;		robotdest[6].y = -1; // Home base (exit course)
//	robotdest[7].x = 5;		robotdest[7].y = -3; // Point 4
//	robotdest[8].x = 0;		robotdest[8].y = -1; // Home base (enter course)
//	robotdest[9].x = 0;		robotdest[9].y = 1; // Home base (enter course)
//	robotdest[10].x = 0;	robotdest[10].y = 11; // Point 5


//	robotdest[0].x = -4; 	robotdest[0].y = 10; // TOP LEFT
//
//	robotdest[1].x = 4;		robotdest[1].y = 6; // MIDDLE RIGHT
//
//	robotdest[2].x = -4;	robotdest[2].y = 2; // BOTTOM LEFT
//	//middle of bottom
//	robotdest[3].x = 0;		robotdest[3].y = 2; // HOME BASE
//	//outside the course
//	robotdest[4].x = 0;		robotdest[4].y = -3; // OUTSIDE
//	//back to middle
//	robotdest[5].x = 0;		robotdest[5].y = 2;  // HOME BASE
//	robotdest[6].x = 4;		robotdest[6].y = 2; // BOTTOM RIGHT
//	robotdest[7].x = -4;	robotdest[7].y = 6; // MIDDLE LEFT
//	robotdest[8].x = 4;		robotdest[8].y = 10; // TOP RIGHT
//	robotdest[9].x = 0;		robotdest[9].y = 9; // TOP MIDDLE
//	robotdest[10].x = -4;	robotdest[10].y = 9; // TOP LEFT
//	robotdest[11].x = 0;	robotdest[11].y = 2; // HOME BASE
	// flag pins
...
//		}
		...
		if ((timecount%200)==0) {
			LCDPrintfLine(1,"%.2f,%.2f", ROBOTps.x, ROBOTps.y);
//			LCDPrintfLine(1, "ArrState: %d", statePos);
//			LCDPrintfLine(2,"%d %.2f,%.2f", is_avoiding_obstacle, robotdest[statePos].x, robotdest[statePos].y);
			LCDPrintfLine(2, "%.2f,%.2f State:%d", robotdest[statePos].x, robotdest[statePos].y, newObs);
		}
		...
		switch(newObs)
		{
			...
			case 2:
				// Drive
				turn = -1 * kpright * rightWallError; // front wall
		...
		if (frontWallError > fronterror)
		{
			...
			//isFrontWall = 1;
			/*if(greenNumPixels > 1800)
			{
				// dead reckoning code
				if(compass > 3000 && compass < 3200)
				{
					top_left++;
					myX = -4.2;
					myY = 9.1;
				...
				// desk corner
				else if (compass > 3300 && compass < 3600 || compass > 0 && compass < 150)
				{
					desk_corner++;
					myX = 5.3;
					myY = 9.8;
...
//		else
//		{
//			// else if right wall detected, avoid it
//			if(rightWallError > 0) {
//				whichAvoid = 2;
//				turn = -1 * kpright * rightWallError; // right wall following
//				vref = .2;//rightVref;
//				if(!is_avoiding_obstacle) {
//					is_avoiding_obstacle = 1;
//				}
...
//
//			else {
//				// if external corner detected, avoid it
//				if(fabsf(corner_error) > corner_error_threshold && fabs(rightWallError) > 700) {
//	//				vref = frontturnvel;
//	//				turn = kpfront * -1 * corner_error;
//					whichAvoid = 3;
//					// clear obstacle avoiding flag
//					if(is_avoiding_obstacle)
//						is_avoiding_obstacle = 0;
//				}
//
//				// else, attempt to go to next xy coordinate (until we see another obstacle)
//				else {
//					// do nothing
//					whichAvoid = 0;
//					if(is_avoiding_obstacle)
